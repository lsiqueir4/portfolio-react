...
- [ ] **DARK-01**: A dark/light toggle button exists in the Header — sun/moon icon, native `<button>`, `aria-label`, `aria-pressed`
- [ ] **DARK-02**: Default theme on first visit follows the browser's `prefers-color-scheme`, with no flash of the wrong theme (SSR-safe: pre-hydration inline script sets `.dark` on `<html>` before paint)
- [ ] **DARK-03**: A manual toggle choice persists across visits via localStorage, overriding the browser default on subsequent loads
- [ ] **DARK-04**: The purple accent color remains the primary accent in both light and dark themes — only background/text tokens invert
- [ ] **DARK-05**: The persistence pattern (read localStorage → fall back to browser signal → allow override → persist) is built as a reusable primitive for Phase 3 to reuse for language

### Language (i18n PT-BR/EN)

- [ ] **I18N-01**: `react-i18next` + `i18next` are integrated (SSR-safe: fixed, matching default language on server and client to avoid hydration mismatch)
- [ ] **I18N-02**: A PT-BR/EN toggle button exists in the Header, next to the dark mode toggle — text labels ("PT"/"EN", not flags), `aria-label`
- [ ] **I18N-03**: Default language on first visit follows the browser's `navigator.language`
- [ ] **I18N-04**: A manual toggle choice persists across visits via localStorage, overriding the browser default on subsequent loads, reusing the Phase 2 persistence primitive
- [ ] **I18N-05**: `document.documentElement.lang` updates when the language changes
- [ ] **I18N-06**: Every user-facing string across Hero, AboutMe, Projects, Contacts, Header, and Footer is translated 1:1 into English — including array-literal content in `data.tsx` files (not just JSX strings), with no rewording of meaning and no new content
- [ ] **I18N-07**: UI-adjacent strings (dates, status labels like "Fev 2022", "Em andamento") ARE translated in the EN version (e.g., "Feb 2022", "In progress") — confirmed as UI elements, not fixed content
- [ ] **I18N-08**: All content, links, and images remain unchanged — only the language of existing text changes

### Documentation

- [ ] **DOCS-01**: `README.md` (PT-BR) is rewritten, highlighting features and the tech stack used
- [ ] **DOCS-02**: `README.en.md` (EN) is created as the English counterpart, with content parity to `README.md`
- [ ] **DOCS-03**: Both README files are cross-linked at the top (language-switch banner) so a reader can jump between versions
